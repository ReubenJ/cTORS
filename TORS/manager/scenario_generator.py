    ...
    def match_trains(self, scenario: Scenario) -> None:
        if self.match_outgoing_trains:
            incoming = [
                t
                for su in [
                    incoming.shunting_unit for incoming in scenario.incoming_trains
                ]
                for t in su.trains
            ]
            outgoing = [
                t
                for su in [
                    outgoing.shunting_unit for outgoing in scenario.outgoing_trains
                ]
                for t in su.trains
            ]
...
class ScenarioGeneratorFromScenario(ScenarioGenerator):
    def __init__(self, n_combinations=100, *args, **kwargs):
        ...
        if self.max_trains_per_track != self.max_trains_per_track:
            warn(
                "parameter 'max_trains_per_track' not supported for ScenarioGeneratorFromScenario."
            )

    def initialize(self, engine, scenario_file: str) -> None:
        super(ScenarioGeneratorFromScenario, self).initialize(engine, scenario_file)
        self.scenario = engine.get_scenario(scenario_file)
        ...
        self.n_disturbances = self._get_number(
            self.n_disturbances, max_disturbances, min=0
        )
        self.n_workers = 0  # self._get_number(self.n_workers, max_workers)
        ...
        if self.enforce_max_length:
            _match_trains(
                [
                    t
                    for su in [
                        incoming.shunting_unit
                        for incoming in self.scenario.incoming_trains
                    ]
                    for t in su.trains
                ],
                [
                    t
                    for su in [
                        outgoing.shunting_unit
                        for outgoing in self.scenario.outgoing_trains
                    ]
                    for t in su.trains
                ],
            )
            ...
            outgoing_trains = []
            for inc in self.scenario.incoming_trains:
                inc.shunting_unit.trains = [
                    t for t in inc.shunting_unit.trains if t in self.valid_trains
                ]
                if len(inc.shunting_unit.trains) > 0:
                    incoming_trains.append(inc)
            for out in self.scenario.outgoing_trains:
                out.shunting_unit.trains = [
                    t for t in out.shunting_unit.trains if t in self.valid_trains
                ]
            ...
            self.valid_trains = [
                t
                for su in [
                    incoming.shunting_unit for incoming in self.scenario.incoming_trains
                ]
                for t in su.trains
            ]
    ...
    def _combination_generator(self):
        combination_generator = itertools.combinations(
            range(self.max_trains), self.n_trains
        )
        combinations_left = self.n_combinations
        seen_combinations = 0
        total_combinations = ScenarioGeneratorFromScenario.__ncr(
            self.max_trains, self.n_trains
        )
        while True:
    ...
    def _select_trains(self, scenario):
        incoming_shunting_units = [
            incoming.shunting_unit for incoming in scenario.incoming_trains
        ]
        outgoing_shunting_units = [
            outgoing.shunting_unit for outgoing in scenario.outgoing_trains
        ]
        ...
        not_incoming_train_units = [
            it for i, it in enumerate(incoming_trains) if not i in combination
        ]
        ...
        scenario.set_incoming_trains(
            [
                incoming
                for incoming in scenario.incoming_trains
                if len(incoming.shunting_unit.trains) > 0
            ]
        )
        scenario.set_outgoing_trains(
            [
                outgoing
                for outgoing in scenario.outgoing_trains
                if len(outgoing.shunting_unit.trains) > 0
            ]
        )
...
class RandomScenarioGenerator(ScenarioGenerator):
    ...
    def _generate_trains(self):
        return [
            Train(i, self._get_random_train_type(), self._get_task_list())
            for i in range(1, self.n_trains + 1)
        ]

    def _get_random_train_type(self):
        options = [
            TrainUnitType(
                "SLT4", 4, 70, 180, 120, 120, 16, 0, 0, "SLT", False, False, True
            ),
            TrainUnitType(
                "SNG3", 3, 35, 180, 120, 120, 16, 0, 0, "SNG", False, False, True
            ),
        ]
    ...
    def _get_task(self):
        options = [Task("Binnenkant reiniging", 1, 50, ["Binnenkant reiniging"])]
        return random.choice(options)

    def _get_parking_tracks(self):
        tracks = [
            track
            for track in self.location.get_track_parts()
            if track.type == TrackPartType.RAILROAD and track.parking_allowed
        ]
        parking_tracks = {}
        for track in tracks:
            side_tracks = track.get_neighbors()
            side_tracks = [
                track for track in side_tracks if track.type == TrackPartType.BUMPER
            ]
    ...
    def _generate_incoming(self, trains):
        ...
        trains_per_type = {
            typ: [train for train in trains if train.type == typ] for typ in types
        }
        ...
        while sum([len(t) for typ, t in trains_per_type.items()]) > 0:
            ...
            next_len = random.randint(
                1, min(len(trains_per_type[typ]), self.max_length)
            )
            selected, trains_per_type[typ] = (
                remaining_trains[:next_len],
                remaining_trains[next_len:],
            )
    ...
    def _generate_outgoing(self, trains, incomings):
        ...
        while sum([len(t) for typ, t in trains_per_type.items()]) > 0:
            ...
            time = max(
                [
                    arrival_times[train.id]
                    + self.location.pathfinder.get_fastest_route(
                        cur_track_id=arrival_track[train.id],
                        dest_track_id=track.id,
                        dest_dir_id=side_track.id,
                    )[1]
            ...
            outgoing = Outgoing(
                id, su, track, side_track, time, outstanding, standing_index, False
            )
            result.append(outgoing)
            trains_per_track[track] += 1
            id += 1
            if (
                not self.max_trains_per_track is None
                and trains_per_track[track] >= self.max_trains_per_track
            ):
    ...
    def _generate_workers(self):
        ...
        workers = [
            Employee(
                ...
                break_windows=[],
                break_duration=0,
                start_track_id=random.choice(tracks),
                end_track_id=random.choice(tracks),
        ...
        cleaning_facilities = [
            f
            for f in facilities
            if any(["reiniging" in tt.lower() for tt in f.task_types])
        ]
    ...
    def initialize(self, engine, scenario_file_string) -> None:
        ...
        if isinstance(scenario_file_string, list):
            ...
            raise NotImplementedError(
                "No scenarios can be obtained from " + scenario_file_string
            )
        to_be_deleted = []
...
def _find_matching_shunting_unit(train, shunting_units):
    return next(
        (
            ...
            if not next((t for t in su.trains if t == train), None) is None
        ),
        None,
    )
...
def _match_trains(incoming, outgoing):
    ...
    available_for_matching = [
        tu for tu in incoming if not tu.id in [otu.id for otu in outgoing]
    ]
